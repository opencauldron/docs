...
Reference images are saved and restored with [Brews](/guides/brews). When you save a brew after a generation that used a reference image, the reference URL is stored with the brew. Loading that brew on the Generate page restores the reference image automatically alongside the model, parameters, and LoRAs.
...
Reference images and their thumbnails are stored in Cloudflare R2, the same object storage used for generated assets. URLs served to the browser are short-lived signed URLs, not permanent links. For storage configuration details, see the [Storage](/guides/storage) guide.
